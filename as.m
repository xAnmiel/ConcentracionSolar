...
for i = 1:length(o_values)
    o_fixed = o_values(i);
    [Cg_values_fixed, L_values_fixed, x_values_fixed, y_values_fixed] = calc_L_Cg_fixed_o(o_fixed);
    hold on
    plot([Cg_values_fixed Cg_values_fixed], [0 L_values_fixed], 'k--')
    hold off
end
...
function [Cg_values, L_values, x_values, y_values] = calc_L_Cg_fixed_o(o_values)
    L_values = zeros(size(o_values)); % inicialización de valores de L (longitud de arco)
    Cg_values = zeros(size(o_values)); % inicialización de valores de Cg
    x_values = zeros(size(o_values)); % inicialización de valores de x
    y_values = zeros(size(o_values)); % inicialización de valores de y
    ...
    for i = 1:length(o_values)
        ...
        x = 0.047*(1+sin(o))*cos(pi/2 - o)/(1-sin(pi/2 - 2*o));
        y = -0.047*(1+sin(o))*sin(pi/2 - o)/(1-sin(pi/2 - 2*o));
